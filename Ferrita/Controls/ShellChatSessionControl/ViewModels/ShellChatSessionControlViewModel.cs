...
namespace Ferrita.Controls.ShellChatSessionControl.ViewModels
{
    public sealed class ShellChatSessionControlViewModel : ObservableObject
    {
        ...
        private bool _isComputerUseMode;
        ...
        public ShellChatSessionControlViewModel(ChatSessionModel session, bool isComputerUseMode)
        {
            _startupContext = ShellChatStartupContext.Empty;
            _chatSessionRepository = new ChatSessionRepository();
            _runtimeService = new ChatSessionRuntimeService();
            _presentationProjector = new ChatSessionPresentationProjector();
            _toolInvocationPresentationService = new ToolInvocationPresentationService();
            _memoryService = new MemoryService();
            _agentAvatarPathsById = LoadAgentAvatarPathsById();
            _shellContextSnapshotXml = string.Empty;

            _sessionModel = session;
            _isComputerUseMode = isComputerUseMode;
            StatusText = BuildInitialStatusText(_sessionModel);
            ...
            {
                ActiveChatSessionExecutionRegistry.Instance.RuntimeEventApplied += OnRuntimeEventApplied;
            }
        }
        ...
        public bool IsComputerUseMode
        {
            get => _isComputerUseMode;
            private set => SetProperty(ref _isComputerUseMode, value);
        }
        ...
        {
            if (_isComputerUseMode && _sessionModel != null)
            {
                var activeSession = ActiveChatSessionExecutionRegistry.Instance.GetActiveSession(_sessionModel.SessionId);
                if (activeSession != null && !ReferenceEquals(_sessionModel, activeSession))
                {
                    _sessionModel = activeSession;
                }
            }
        ...
        private void OnRuntimeEventApplied(object? sender, ChatSessionRuntimeEvent e)
        {
            if (_sessionModel != null && string.Equals(e.SessionId, _sessionModel.SessionId, StringComparison.OrdinalIgnoreCase))
            {
                var dispatcher = Application.Current?.Dispatcher;
                if (dispatcher == null)
                {
                ...
                {
                    dispatcher.BeginInvoke(new Action(() => ApplyRuntimeEvent(e)), System.Windows.Threading.DispatcherPriority.Normal);
                }
            }
        }
        ...
        private void RefreshMessagesFromTranscript()
        {
            ...
            if (dispatcher != null && !dispatcher.CheckAccess())
            {
                dispatcher.BeginInvoke(new Action(RefreshMessagesFromTranscript), System.Windows.Threading.DispatcherPriority.Normal);
                return;
            }
